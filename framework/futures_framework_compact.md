...
**DYNAMIC-pool**　池内范围；核心MA/RB；备选M/SR/CF；信号AU（AU2612）/SC（SC2611-SC2612，9/16起）；BU、EC不入池；铜零TC不重复作独立收缩驱动；能源→MA、黑色→RB月差；利率无当前可执行表达；农品依独立证据；M 无已许可具体策略=research_only 范围（仅观察）；例外：MA/M限隔夜；RB/SR/CF常规；AU/SC不交易；MA/SR/CF=CZCE，RB/AU=SHFE，M=DCE，SC=INE；解除/更新：用户新裁决。 黄金信号；不交易；不评估建仓；例外：不换AG/不ATR自动复活；解除/更新：账户/预算变更另议。 原油信号；不交易；服务①与MA护栏（①改判中断证真后本腿仍为唯一量化锚）；例外：不因重算激活；解除/更新：另行池许可。

<a id="r-dynamic-frequency"></a>
**DYNAMIC-frequency**　高密度簇；半周；9/21与9/29公告节点±1日及9/30长假T-1日级；按频率复核；例外：常规周度；铁水阈值不再作频率触发；解除/更新：密度下降。
...
- 本表来自本期快照（脚本 v1.15；最新行情日 9/18；20 日均量、剩余 td、分位、样本）
- 中位td非硬触发
- 按本表目标与[ROLL-direction](#r-roll-direction)/[ROLL-structure](#r-roll-structure)/[ROLL-target](#r-roll-target)复核
- 例外：AU/SC只信号
- 解除/更新：每次换月与快照更新时重核。 MA
- 结构对 MA2701-MA2705（9/16 已下滚；9/18 分位 100、价差+312、样本 41/41/41 complete；1td −18/5td +30/10td +123）
- 执行腿/#30 护栏判定腿/A 近腿=MA2701（76td、540,023 手、settle 2977、H20 3154/L20 2604、周涨 −2.90%、2ATR 1,843→预检上界 1 手、ATR250 分位 82.9 高波）
- MA2705 155td、19,628 手
...
- 当前对 RB2701-RB2703（9/10 切换；9/18 分位 56.1、价差−8、样本 complete→未触发 no_signal）
- RB2701 77td、669,156 手（ATR 分位 72.7 常规）
- RB2703 113td、27,445 手
- 准备对 RB2703-RB2705 仅取样（RB2705 13,672 过#2、156td；分位 61.8）
- 下次：RB2701 触#1（≈2026-12-18）或主力换月
- 例外：旧 CU/AL/AG 换腿指令作废，AU 已用 2612
- 解除/更新：脚本§1 输出后按[INPUT-spread](#r-input-spread)验收并回填。 M/SR/CF
- M2701 77td、1,600,298 手（ATR 分位 99.4 高波；HV20/60 1.21；2ATR 1,114→预检上界 2 手）
- SR2701 76td、470,759 手（70.1 常规；settle 5326、周涨−2.22%、ATR20 53.31、MA20 5399.8；2ATR 1,086→4 手），SR2701-SR2705 分位 0.0/价差−78→未触发
- CF2701 76td、347,245 手（22.0 低波；settle 15795、周涨−3.98%、ATR20 219.42、MA20 16672、H20 17345/L20 15715、ADX 30.1；2ATR 2,244→2 手；均为 PLAN-B 输入，pre_window_low5 缺）
...
- 不为次主力标签提早换弱月
- 解除/更新：当日复核。 AU/SC
- AU2612 55td、80,370 手、settle 946.18、2ATR 39,479（0 手，≈净值 26%）→约 11 月中旬→AU2702
- SC2611-SC2612（9/16 已随主力换月；9/18 +43.5/分位 100、1td −9.7/10td +17.9；SC2611 23td、95,132 手；SC2612 44td、22,661 手；SC2611 结算周涨 −0.30%）→下次主力换月→SC2612-SC2701
- 只更新信号，不开仓
- 例外：SC 不存在挂牌月份按[ROLL-SC](#r-roll-sc)回退
- 解除/更新：实际主力状态。

| 品种 | 主力月集合(实测) | 市场换月中位剩余td | 当前腿 | 20日均量(9/18实测) | 剩余td(9/18实测) | 首次触发#1 | 下一目标腿 |
|---|---|---|---|---|---|---|---|
...
**DYNAMIC-regime**　当前双轴/利率/政策/黑色状态（供本期逐项复评）：

- ①=**中断证真**（本期判据：要件一再袭船/布雷 9/8–9/9 多源✓、IRGC 9/17 再袭油轮 Trend 单源待多源；要件二脚本§1 SC 近端 back 9/16 +56.7、9/18 +43.5，分位 100、10td +17.9；反向要件未出现——9/14 萨拉拉会 9/13 推迟无新日期）
- 反向监测项（参考级，不构成回改）=9/17 美军护航 40 艘（CENTCOM 1,800 万桶 vs Vortexa 4/Kpler 12 conflicting）、9/18 沙特东西管道半恢复（2–2.5 百万桶/日；管道 1,200km、4–5 百万桶/日，9/10–11 遇袭）+苏哈尔过驳、布油 9/14 触 108→9/18 103.87（周 −0.7%）、WTI 100.30、back 1td −9.7、SC 单周 +16%→−14%（9/17 −6%/9/18 −9%）
- IEA：2026 需求 −250 万桶/日、海湾 >1,000 万桶/日关停、库存 −5.07 亿桶（8 月 −9,500 万）
- 回改标准=官方停火/重开或伊朗许可-收费机制正式落地＋back 回落（[GATE-calm](#r-gate-calm)）→缓和信号·量化未确认/僵局期
- 再袭船·管道再袭＋back 再走阔→维持
- **改判不改变方向许可**：空头[0.3#26](#r-gate-26)否决、多头[0.3#16](#r-gate-16)/[0.3#30](#r-gate-30)/[CARD-MA-week](#r-card-ma-week)/D8/D12、SC 正套=信号席不建仓、fade（research_only）不适用
- 供给冲击第三形态"绕行通道（沙特东西管道 9/10–11 遇袭、9/12 关停、9/17–18 半恢复）袭击/修复"仅记录，不新设轴/门
- fed_state=加息落地·连续加息指引（9/17 02:00 加息 25bp 至 3.75–4.00%、12-0、点阵 16/18 年内再加、2026 末中位 4.1%、10 月 49%→57%、10Y 5.00%、美元 100.2）
- D13=加息落地·压制逻辑重写（常态扣减 −0.3/×0.5/C 门槛 4.8 照存档；复归档条件=10/27–28 FOMC 按兵或转鸽＋美元连涨中断周度确认）
- AU 信号卡=预期兑现型反弹评估（决议后低点约 4,270→9/18 4,368.60、沪金 948.52；4404 回收=第一观察；<4000 见[PLAN-AU-break](#r-plan-au-break)）
- ②''=形式过门·结构核对未过，第二读数"反弹但未修复"（9/15 硬数据弱现实：工业 +5.2%/社零 +0.4%/固投 −7.2%/地产 −19.9%；沪指周 +0.61%（9/18 3911.87、+0.94%）、创业板周 +1.52%、9/18 成交 2.09 万亿（放量 2,564 亿）但日均 1.81 万亿、两融 26,308 亿（+74 亿）；固投扣除地产 −4.2%）→×0.5 维持，下一裁决 10 月中旬 9 月硬数据
- ③=判决期（多晶硅减产方案未定；调减数据见[DYNAMIC-deadline](#r-dynamic-deadline)）
- ④休眠
- RB=旺季证伪侧、多头评估冻结，负反馈检验点=**落地但临界**（纠错：焦炭第五轮 9/8 发起、9/10 落地 100/110；同源钢厂亏损扩大、减产计划增多、第六轮观望；不执行延续/不执行启动；下一裁决=钢厂减产落地/第六轮）
- 交易所风控=收紧（郑商所 9/21、9/29 分级提保扩板，见[DYNAMIC-events](#r-dynamic-events)）
- 低敞口=命中（高密度簇＋交易所公告±1日＋D12 高波层）
- 例外：以上不是本次已核触发
- 长期许可不得因缺数自行解除
- 到期事件无新证据不得续冻
- 池外品种状态只作背景
- 解除/更新：10/9 长假复盘、10/27–28 FOMC T+1 逐项复评。

<a id="r-dynamic-cpi"></a>
**DYNAMIC-CPI**　利率链分支（"加息＋连续加息指引"分支已于9/17到期执行；当前=10 月 FOMC 三剧本）：

- 已核：加息 25bp 至 3.75–4.00%（12-0；声明"通胀仍然偏高"）、点阵 16/18 年内再加、2026 末中位 4.1%（前 3.8%）、10 月再加息 49%（记者会后）→57%（9/18）、年内至少一次 87%、两次 37%、10Y 5.00%、美元 100.2（盘中 100.4；FOMC 日 +1.14%）、现货金决议前 4,328.39→决议后低点约 4,270→9/18 4,368.60（周 −0.4%）、沪金主力 948.52（+1.41%）
- AU 信号卡=加息落地·预期兑现型反弹评估
...
- AU 不交易、金融属性品种池外→本项无执行表达
- 不把情景推演写成必然概率
- #29 9/11–9/18 已归档，10/27–28 按 T−10 核重入
- 解除/更新：10/27–28 FOMC 落地，T+1 重写 fed_state/D13/AU 卡。

<a id="r-dynamic-iron-sample"></a>
**DYNAMIC-iron-sample**　黑色需求侧判决输入；钢材总库存周报（公开来源，须完整年份/观测日/口径；跨年读数已撤回不用）；本周 Mysteel 五大品种同口径数值未取得→temporary_gap 子项；兰格&找钢 9/16 口径可用（重点品种总库存 2,101.77 万吨 −29.13、建材总库存 1,061.27 −31.43、建材产量 417.87 −11.13、表需 449.30）但与 Mysteel 不同源不可比；焦炭第五轮 9/10 落地（纠错）为背景；247 家铁水日均/盈利率为参考旁证，不记缺口、不作补核必办；例外：仅依赖该因子的模型必需；不同样本/跨年数据不冒充；独立国内月差不因缺库存失败，已知反证须处理；RB2701-RB2703 本周分位 56.1 未触发，本项仅为背景；解除/更新：Mysteel 公开周报到位后重核。

## 3.4 本月事件与固定风险窗

<a id="r-dynamic-events"></a>
**DYNAMIC-events**　当期事件簇（9/21–10/28）：

- **9/21（周一）郑商所提保扩板第一节点**：结算起 MA2610 涨跌停 9%、MA2611 保证金 9%/涨跌停 8%（公告±1日=9/18–9/22 受影响品种新开门槛+0.3、公告日核占用；[REG-exchange](#r-reg-exchange)/[0.3#10](#r-gate-10) ×0.8）
- **9/29（周二）第二节点**：甲醇全部合约保证金 10%、涨跌停 9%（±1日=9/28–9/30）
- 10/8 恢复交易后自持仓量最大合约未出现涨跌停单边市的首个交易日结算起回落
- 另有"9/21 起 PTA/甲醇/PX 单独通知"未核、INE 对 SC 阴性待核
- **9/30 俄柴油禁令到期**=俄乌轴质变节点（Vedomosti 9/16 报道拟延至 10 月底、政府未官宣=报道级候选，官宣日才按[REG-geop](#r-reg-geop)质变日执行）
- **9/30 同为国庆长假 T−1**：全品种方向性单边对冲或减至×0.5（[MANAGE-event](#r-manage-event)），[0.3#22](#r-gate-22)跨假期敞口>30% 风险预算减仓
- **10/1–10/8 长假**（闭市；10/4 OPEC+ 与地缘 headline 在闭市期发生，10/9 为首个响应日，按[TRIGGER-D](#r-trigger-d)落地次日确认与双向跳空预案；低敞口条款 10/9 复评）
- **10/4 OPEC+**：按兵→延续/增产或闲置产能释放→回吐触发、MA 存量 10/9 检视
- **10/27–28 FOMC**：10 月再加息 49–57%，三剧本见[DYNAMIC-CPI](#r-dynamic-cpi)
- 官方发布时间与国内 T−n 待下月按[CAL-timestamp](#r-cal-timestamp)核（美东 10/28 14:00≈北京 10/29 02:00→国内 T 大概率 10/29）
- AU/AG [0.3#29](#r-gate-29) 按 T−10 核重入
- 已落地归档：9/14 萨拉拉会（推迟无新日期）、9/15 统计局 8 月硬数据、9/17 FOMC、9/16 MA/SC 换月、9/10 PPI、9/11 CPI、9/6 OPEC+、9/10 RB 换月、质变日 9/8–9/9
...
|---|---|---|---|
| 郑商所提保扩板第一节点（交易所公告日程） | 2026-09-17 公告 | 2026-09-21 结算起 | 9/21；MA2610/2611 分级；公告±1日=9/18–9/22 门槛+0.3、×0.8、核占用 |
| 郑商所提保扩板第二节点 | 2026-09-17 公告 | 2026-09-29 结算起 | 9/29；甲醇全合约保证金 10%/涨跌停 9%；±1日=9/28–9/30；10/8 后按条件回落 |
| 俄柴油禁令到期 / 国庆长假 T−1 | 2026-09-30（政策到期；延期报道级） | 2026-09-30 | 9/30；官宣日=质变日；长假 T−1 对冲或×0.5、#22 跨假期敞口 |
| 国庆长假 | — | 2026-10-01 至 10-08 闭市 | 10/9 首个响应日；假期内 10/4 OPEC+ 与地缘 headline 集中定价 |
| OPEC+ 会议 | 2026-10-04（当地，时刻未核） | 2026-10-04 | 闭市期→10/9 按落地次日确认；MA 存量 10/9 检视 |
| FOMC 决议 | 2026-10-28 14:00 美东（待官方日历核） | 约 2026-10-29 02:00（待核） | 约 10/29（待核）；T−3/T−1/T+1 下月写死；#29 T−10 |

<a id="r-dynamic-gap"></a>
**DYNAMIC-gap**　金银事件窗口；9/10–9/17=1.3 已到期归档；10/27–28 FOMC±1→1.3（日期待核）；地缘质变±2→1.3；化工链 MA：①中断证真期 1.2 维持（反向质变=官方停火/重开/许可-收费机制落地为主要跳空源，绕行通道再袭/修复为第二源），9/21–9/30 提保扩板期与跨长假按扩板后涨跌停口径（9%/8%）核压力场景；例外：AU 信号/AG 休眠仅历史存量；解除/更新：窗口后核。

<a id="r-dynamic-freezes"></a>
**DYNAMIC-freezes**　已过/当前冻结与护栏命中（本期快照，行情日 9/18）：

- fixed_risk_window：D12 全品种方向性 9/11–9/17 与 #29 AU/AG 9/11–9/18 均到期归档，下一提前窗待 10 月官方日历
- **#30：MA2701 判定腿 9/14–9/18 逐日结算涨跌未取得→本周命中判定 unknown（周涨 −2.90% 不推断单日；主力 MA2610 9/16 日内 +4% 报 3559/收 +3% 与 SC 信号腿不替代判定腿；脚本 v1.16 §2b.1 新列重跑后核）
- SC 信号腿 9/14 +11%（多向）、9/15 +6%、9/17 −6%、9/18 −9%（空向）三次单日≥5%（收盘口径转引，信号席仅记录）**
- **MA周涨护栏：SC2611 结算周涨 9/11→9/18 −0.30%（快照实测两端 settle）→未命中 >5%/>8%
- 上一读数 +22.56%（9/9→9/16）的多头冻结·清仓处置已到期执行（条件式）不续延，亦不因未命中恢复多头新开（#16 仍否决）**
- **D8（脚本§2d，主力 MA2610）：周涨 +0.93% 上尾位 66.7→无档（9/16 扣分档解除）
- #20 未命中（dist_H250 5.61%）
- RB −0.42% 上尾 45.1 无档
- SR −2.22% 下尾 94.1、CF −3.98% 下尾 98.0=空头否决档（B 仅做多，不适用）
- AU +0.12%、SC −0.30% 无档
- 全部 153/153 样本**
- **D12（快照实测）：MA2701 高波层（ATR250 分位 82.9、HV20/60 1.22）×0.5+门槛+0.3、M2701 高波层（99.4）、RB 72.7/SR 70.1 常规、CF 22.0/AU 33.0 低波
- 池内极差 77.4（M2701↔CF2701）=错位→逐品种独立核参，方向性单边隔夜须过当日 ATR 重校准**
- 交易所：郑商所 9/17 公告→2610/2611 自 9/21、甲醇全合约自 9/29 ×0.8 缓冲，公告±1日门槛+0.3
- #16 低敞口限隔夜池方向性单边新开否决当期有效（高密度簇＋交易所公告＋D12 高波层）
- #26 能源链方向性空头否决当期有效（①中断证真、未证缓和）
- 质变日：9/14 萨拉拉推迟未按质变日执行，IRGC 9/17 单源声明未执行±1 冻结（待多源）
...
**DYNAMIC-deadline**　调减数据硬截止已到期；③自然日硬截止2026-09-12当日无可核数据→判决点"无法核验"；首个交易日2026-09-14记录生效；光伏端F腿（SI/PS的F远月表达）新开冻结（中性：非罚则、无叙事扣减、存量不动）；9/18 多晶硅减产力度与执行方案仍未定、报价体系未恢复；例外：SI/PS池外，只改本状态与扫描行，不影响池内候选；无数据非未达标；解除/更新：可核数据出现即按[0.3#23](#r-gate-23)原条款裁决。
...
- ③甲醇港口/华东社会库存同口径数值与装置复产进度、Mysteel 五大品种同口径钢材总库存（研究方，2026-09-25；各最多两个公开入口，本周均阴性；到期仍缺→继续 temporary_gap 子项，不转 research_only）
- ④seasonal_plan.py：SR-summer planned_exit 真实交易日映射（≈9/23）与期限可行性、CF-autumn pre_window_low5（8/25–8/31）/[PLAN-B-history](#r-plan-b-history)/Entry/SL/TP1/净R、rule_version=B-v2.24 登记（研究方，2026-09-23）
- ⑤郑商所 9/21 起 PTA/甲醇/PX 单独通知与 INE 公告原文（研究方，2026-09-21）
- ⑥①"中断证真"反向监测（下次 change-analysis，2026-09-26）：IRGC 再袭船多源确认、back 方向（基准 +43.5/分位 100）、官方停火/重开或许可-收费机制落地、沙特管道全量恢复、俄禁令 9/30 前官宣
- ⑦影子计划 SP-2026-09-19-MA-A-short-spread（MA2701-MA2705 空价差：限价 +312/SL +380/TP +120，有效期 9/25，最晚退出 10/23，乘数 10，成本 48；登记时阻断 #5）与 SP-2026-09-19-MA2701-long-D（MA2701 止损单 3155/SL 3018/TP 3429，同期限，成本 30；登记时阻断 #16、D12 高波）由脚本§5 结算（首个可用日线 2026-09-22），只供复评 #5/#16/D12
- ⑧当期账户/持仓/挂单/费用/保证金快照（账户持有人，执行核验阶段）
- ⑨MA2705-MA2709 准备对=temporary_gap（MA2709 量缺失、样本 24/21/22），到期不转 research_only
- SR 产销率反向（广西截至 8 月 80.56%、同比 −8.48pp、工业库存 +78.74 万吨；广西现货 9/18 5090–5170、ICE 原糖 17.42）/窗口尾段、CF 供给端反证（储备棉 9/18 成交率 96.37% 本轮首见 <100%、累计 99.15%/358,199 吨；籽棉开秤预期 7.0–7.5 元/公斤；#5(b) 已核 fail）、M 无许可路由=备选未激活，不作必办
- 例外：池外旧裁决不占当前必办
...
- ①护航 40 艘＋沙特管道半恢复≠①缓和证真（反向要件只认官方停火/重开或伊朗许可-收费机制落地＋back 回落；CENTCOM vs Vortexa/Kpler conflicting 为参考级；①已改判中断证真、#26 继续、fade 不开放、SC 信号席不建仓）
- ②①改判≠方向许可变化（多头仍受 #16/#30/周涨/D8/D12；不复活已存档建仓路径；反向要件出现时按同一标准回改，非摇摆）
- ③加息落地＋连续指引≠贵金属趋势级回吐已定（决议后 4,270 两日反弹至 4,368.60；10Y 5%/美元 100 为 18 年/七周高；10 月仅 49–57%；AU 信号席不交易；#17/反向 E 不开放；D13 扣减参数不变）
- ④SC −14%/布油周 −0.7%≠能源空头或 E 回归开放（绕行修复驱动非事件落地；①未证缓和；#26；SC 两次≥5% 空向若开放亦触 #30；提保扩板放大单腿风险）
- ⑤SC +11%/甲醇主力 +4%≠追多或 continuation 许可（continuation 未许可；确认定义仅 draft；#16；D12 高波 82.9；3559 为 MA2610 非 MA2701；D8 无档/周涨未命中不恢复新开）
- ⑥第五轮落地（纠错）≠原料主线延续或 RB 多头解冻（钢厂减产计划增多→临界；固投 −7.2%/地产 −19.9%；兰格与 Mysteel 不同源；RB 56.1 未触发）
- ⑦A 股 9/18 放量 2.09 万亿≠②''修复（日均 1.81 万亿；×0.5 维持池外）
- ⑧交易所提保扩板≠结构豁免或新护栏（既有 ×0.8/门槛+0.3 进入适用；结构不豁免组合上限与双腿占用）
- ⑨MA2701-MA2705 分位 100/+312≠必回归（#5(b) 反证；影子计划仅为规则复评假设；港口库存数值仍缺=temporary_gap 子项）
- ⑩俄禁令"延至 10 月底"≠升级已落地（未官宣；9/30 处置依据是长假 T−1）
- ⑪significant≠稳定性判断（三项门/状态变更＋回填＋纠错，规则本体零改动；10/9 与 10 月 FOMC 后级别不预设；护栏命中数/连续空仓/一次踏空不证明规则有效或过严）
- 首要纪律风险（6.9）：双向 headline 周把任一侧单日走势外推为方向（−9% 追空/+11% 追多）、把护航/管道修复当①缓和证真解除 #26、加息落地后抄底贵金属或押"5%/100 见顶"、用第五轮落地外推原料主线做多黑色、9/21–9/30 提保扩板期与长假前裸持跨假期单边（9/30 T−1 对冲或×0.5、#22）、把分位 100 当回归信号或把库存数值缺失当 A 不可研究、未过当日 ATR 重校准隔夜（MA2701 82.9/M 99.4 高波、极差 77.4）、被动等追保、把周涨未命中/D8 无档当多头新开窗口、把池外品种当机会
...
- 主="护航 40 艘＋沙特管道半恢复＝海峡事实重开、油价见顶回落"（MA/SC 信号/能源链空头侧；破裂=再袭护航船队/管道再遭袭/伊朗正式收费机制落地日 +5% 级跳空）
- 新="加息落地＋连续加息指引＝贵金属趋势级回吐已定、5%/100 只是起点"（AU 信号，联动池外金融属性；只影响 fed_state/D13 状态）
- 改版="甲醇分位 100＝必回归"与"近月无限强"双向（MA；主力 +0.93% vs MA2701 −2.90%、装置复产前瞻、提保扩板/长假、确认定义 draft）
- 改版="第五轮落地＝原料主线延续＝旺季确认"（RB；钢厂减产计划增多、弱现实硬数据、Mysteel 缺数）
- 新="A 股放量站上 2 万亿＝资金结构已修复"（内需商品链/权益 beta 池外；日均 1.81 万亿）
- 延续="自律锁价＝价格底/反内卷全面量化"（SI/PS/RB/AL/LC，除 RB 外池外；减产方案未定）
- 已证伪存档不评分："LC 需求拉动型去库＝重启候选"
- 例外：只对实际适用候选评分
- 池外不入执行分母
- 解除/更新：下周换版。
