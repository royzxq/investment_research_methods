...
<a id="r-reg-geop"></a>
**REG-geop**　地缘双轴任一质变；霍尔木兹停火或正式重开官宣、正式关闭、扩大打击或再袭船·布雷；俄乌停火/禁令扩汽油或9-30延期或解除/更大打击；当日及次日方向性单边新开冻结，双向跳空预案；MA结构依专属±1另核；例外：通行量实测为参考，不构成质变形态；不是新增方向许可；已执行旧事件不无新证据续冻；解除/更新：当前质变窗口退出并复核其他触发。
...
**RISK-regime**　低敞口/常规；常规=净值3.5%;固定low_exposure_cap=5000；低敞口current=min(normal,5000),常规current=normal;无regime_factor；例外：不加入单笔乘积;净值降低不得超normal,净值增加低敞口仍封顶5000；解除/更新：全部触发复评。 全部交易；单笔及常规组合均净值3.5%；单笔及组合分别核验;当前组合另按状态公式；例外：计划止损不是最大亏损;固定5000只封顶低敞口组合；解除/更新：账户更新重算。
...
<a id="r-risk-final"></a>
**RISK-final**　可核整数交易容量；final_lots=min(risk_lots,margin_capacity_lots,position_limit_capacity_lots)；final_lots;输出normal/current及low_exposure_cash_cap;旧portfolio_risk_cap为current别名;删除regime_factor输出；例外：容量未知null;容量0阻断;数值输出不代替全规则许可；解除/更新：容量更新。
...
**CARD-RB-manage**　成本端验证反向/月差收敛；减50%/了结；D14适用（[D14-restart](#r-d14-restart)/[D14-stock](#r-d14-stock)）；按品种管理；例外：铁水创新高、铁水<230为参考不触发；结构需自身SL；解除/更新：证据变化。 RB周末；周五50%；减仓;需求证弱（公开库存）转结构；例外：须结构许可；解除/更新：周末过后。
...
**DYNAMIC-version**　当期配置截止 **2026-09-26**（light 状态换版：交易所休市日历纠错、交易所风控全池、①维持记录、负反馈启动、MA 路线结案与确认定义 v1；方法版本 v2.28，规则/参数/门状态零改动），FRAMEWORK_UPDATED_AT/AS_OF_DATE=该日；**MARKET_SNAPSHOT_AS_OF=2026-09-18（stale）**：本周无新快照，最近可用 research/2026-09-19-data-snapshot.txt（脚本 v1.15 实测，落后最近已完成交易日 2026-09-24 四个交易日），依赖快照的分位/价差/样本/ATR/D8/周涨/#30/影子结算一律 unknown，不推算、不沿用；脚本 v1.17（仅 EVENTS 日历配置随动）为分支拟议，未联网实测。研究默认 public_data；本期公开证据为搜索摘要转引（多源官方 verified、单源/冲突项降级），不冒充原文核验。配置不证明持仓，旧快照不证明新一轮触发；下次 change-analysis 更新，**10/8 长假复盘**逐项复评。

<a id="r-dynamic-caps"></a>
**DYNAMIC-caps**　账户配置；用户裁决日2026-09-06；ACCOUNT_SIZE=150000；RISK_BUDGET/PORTFOLIO_RISK_CAP=5250；LOW_EXPOSURE_RISK_CAP=5000；示例#22=1,575元；RISK_REGIME=low_exposure（当期命中：高密度簇 9/28 headline 首日/9/29 提保第二阶段/9/30 T-1/10/1–10/7 休市/10/8 复盘/10/27–28＋交易所公告±1日（9/28–9/30 全池）＋D12 高波层（上一实测 MA2701 82.9，本周无快照未刷新）；组合current=5000）；配置上限按RISK-regime；实际账户核验后重新计算；例外：实际仓位未知;独立风险系数/门不变；解除/更新：10/8 长假复盘按0.1复评全部有效触发后才可解除。

<a id="r-dynamic-pool"></a>
**DYNAMIC-pool**　池内范围；核心MA/RB；备选M/SR/CF；信号AU（AU2612）/SC（SC2611-SC2612，9/16起）；BU、EC不入池；铜零TC不重复作独立收缩驱动；能源→MA、黑色→RB月差；利率无当前可执行表达；农品依独立证据；M 无已许可具体策略=research_only 范围（仅观察）；例外：MA/M限隔夜；RB/SR/CF常规；AU/SC不交易；MA/SR/CF=CZCE，RB/AU=SHFE，M=DCE，SC=INE；解除/更新：用户新裁决。 黄金信号；不交易；不评估建仓；例外：不换AG/不ATR自动复活；解除/更新：账户/预算变更另议。 原油信号；不交易；服务①与MA护栏（①维持中断证真，本腿为唯一量化锚；本周无快照 back 方向 unknown）；例外：不因重算激活；解除/更新：另行池许可。

<a id="r-dynamic-frequency"></a>
**DYNAMIC-frequency**　高密度簇；日级——假期前仅 9/28（周末 headline 首个交易日）、9/29（郑商所/大商所第二阶段公告±1日）、9/30（长假T-1/俄禁令/9月PMI）三个交易日；9/25–9/27 与 10/1–10/7 休市；10/8 复盘；按频率复核；例外：常规周度；铁水阈值不再作频率触发；解除/更新：密度下降。

<a id="r-dynamic-mult"></a>
**DYNAMIC-mult**　合约规格；MA/SR/RB/M：multiplier=10、tick=1、tick_value=10；CF：5、5、25；AU：1000、0.02、20；SC：1000桶、0.1、100；价差/止损人民币换算；例外：价格和单位须核；信号仅预检；品种复活先同步框架与脚本规格；解除/更新：参数核定变化。
...
**DYNAMIC-roll-table**　执行日/主力换月核验：

- 本表来自上一快照（脚本 v1.15；最新行情日 9/18；**本周无新快照**：20 日均量、分位、样本、指标均为过期对照，不作本周判定；剩余 td 按日历口径=表值减 9/21–9/24 四个交易日）
- 中位td非硬触发
- 按本表目标与[ROLL-direction](#r-roll-direction)/[ROLL-structure](#r-roll-structure)/[ROLL-target](#r-roll-target)复核
- 例外：AU/SC只信号
- 解除/更新：每次换月与快照更新时重核。 MA
- 结构对 MA2701-MA2705（9/16 已下滚；上一实测 9/18 分位 100、价差+312、样本 41/41/41 complete；本周 unknown；国内路线论证结案未成案、确认定义 A-MA-2701-2705-v1 冻结 9/28 起前瞻）
- 执行腿/#30 护栏判定腿/A 近腿=MA2701（9/24 日历口径 72td；上一实测 540,023 手、settle 2977、H20 3154/L20 2604、周涨 −2.90%、2ATR 1,843→预检上界 1 手、ATR250 分位 82.9 高波——过期对照）
- MA2705 151td（9/24）、19,628 手（9/18）
- 准备对 MA2705-MA2709 仅取样（分位 100 仅参考；MA2709 20 日均量缺失、样本 24/21/22<33=temporary_gap，不授许可）
- 下次：MA2701 触#1（≈2026-12-17）或主力换月→MA2705-MA2709 当日核#2
- 方向性执行腿在 MA2701 剩余 td<50（约 11 月初，脚本§0 预警）→MA2705
- 例外：不把结构走势自动翻译为买远月
- 解除/更新：当日复核并回填 0.2。 RB
- 当前对 RB2701-RB2703（9/10 切换；上一实测 9/18 分位 56.1 未触发——本周无快照 unknown，上周 no_signal 不沿用）
- RB2701 73td（9/24）、669,156 手（9/18；ATR 分位 72.7 常规）
- RB2703 109td（9/24）、27,445 手（9/18）
- 准备对 RB2703-RB2705 仅取样（RB2705 13,672 过#2（9/18）、152td；上一分位 61.8）
- 下次：RB2701 触#1（≈2026-12-18）或主力换月
- 例外：旧 CU/AL/AG 换腿指令作废，AU 已用 2612
- 解除/更新：脚本§1 输出后按[INPUT-spread](#r-input-spread)验收并回填。 M/SR/CF
- M2701 73td（9/24）、1,600,298 手（9/18；ATR 分位 99.4 高波；HV20/60 1.21；2ATR 1,114→预检上界 2 手）
- SR2701 72td（9/24）、470,759 手（9/18；70.1 常规；settle 5326、周涨−2.22%、ATR20 53.31、MA20 5399.8；2ATR 1,086→4 手），SR2701-SR2705 上一分位 0.0（本周 unknown）；SR-summer planned_exit 9/22 已过→结案
- CF2701 72td（9/24）、347,245 手（9/18；22.0 低波；settle 15795、周涨−3.98%、ATR20 219.42、MA20 16672、H20 17345/L20 15715、ADX 30.1；2ATR 2,244→2 手；均为过期的 PLAN-B 输入，需新快照；pre_window_low5 缺）；CF-autumn planned_exit 10/23
- RB2701 HV20/60 1.24（<1.3）
- M 12/18→M2705
- SR 12/17→SR2705-SR2709
- CF 12/17→CF2705
- 主力换月可先到
- 目标当日核#2
- 例外：CF2705 不合退 CF2703
- 不为次主力标签提早换弱月
- 解除/更新：当日复核。 AU/SC
- AU2612 51td（9/24）、80,370 手（9/18）、settle 946.18、2ATR 39,479（0 手，≈净值 26%）→约 11 月中旬→AU2702
- SC2611-SC2612（9/16 已随主力换月；上一实测 9/18 +43.5/分位 100、1td −9.7/10td +17.9；SC2611 19td（9/24）、95,132 手；SC2612 40td、22,661 手；SC2611 结算周涨 9/11→9/18 −0.30% 已过期，9/17→9/24 unknown；媒体收盘 9/24 +5.02% 仅记录）→下次主力换月→SC2612-SC2701
- 只更新信号，不开仓
- 例外：SC 不存在挂牌月份按[ROLL-SC](#r-roll-sc)回退
- 解除/更新：实际主力状态。

| 品种 | 主力月集合(实测) | 市场换月中位剩余td | 当前腿 | 20日均量(9/18实测;本周无快照) | 剩余td(9/18实测;9/24口径=减4) | 首次触发#1 | 下一目标腿 |
|---|---|---|---|---|---|---|---|
| MA | 01/05/09(10=过渡月) | 21 | 结构:MA2701-MA2705(9/16 已下滚;分位 100、价差+312);执行腿/护栏判定腿:MA2701 | 540,023 / 19,628 | 76 / 155 | ≈2026-12-17(MA2701 近腿);执行腿剩余 td<50(约 11 月初)→MA2705 | 结构→MA2705-MA2709(MA2709 量缺失、样本 24/21/22<33=temporary_gap,不授许可)或按主力换月 |
| RB | 01/05/10 | 25 | **RB2701-RB2703**(9/10 切换;分位 56.1、样本 complete;未触发) | 669,156 / 27,445 | 77 / 113 | ≈2026-12-18(RB2701 近腿) | RB2703-RB2705 或按主力换月(RB2705 13,672 过#2;准备对分位 61.8 仅取样) |
| M | 01/05/09 | 25 | M2701 | 1,600,298 | 77 | 2026-12-18 | M2705(225,557;9/4 值) |
| SR | 01/05/09 | 24 | SR2701-SR2705(分位 0.0;未触发) | 470,759 / 40,611 | 76 / 155 | 2026-12-17 | SR2705-SR2709 |
...
| SC(信号) | 全月01-12 | 9 | SC2611-SC2612(9/16 已随主力换月下滚;分位 100、价差+43.5) | 95,132 / 22,661 | 23 / 44 | 已过(注c,不受#1) | SC2612-SC2701(下次主力换月日) |

## 3.3 历史状态的复评边界

<a id="r-dynamic-regime"></a>
**DYNAMIC-regime**　当前双轴/利率/政策/黑色状态（供本期逐项复评；截至 2026-09-26）：

- ①=**中断证真（9/26 复核维持）**（要件一再袭船/布雷：9/21 英旗油轮 La Stephanie 与一艘 LPG 船遭不明射弹击中（UKMTO，多源）再命中；要件二脚本§1 SC 近端 back：本周无快照 unknown，上一实测 9/18 +43.5、分位 100；反向要件未出现——伊朗 9/25 七日重开方案（经卡塔尔转交）被 Trump 9/26 拒绝并启动"经济 D 日"、"不在任何谈判中"=谈判 headline 非裁决依据）
- 反向监测项（参考级，不构成回改）=七日方案是否官方化/分阶段谈判重启、沙特东西管道 9/22 低速复运（全量需数周）、9/24 胡塞导弹袭延布/利雅得（联军拦截 6 枚，无损失确认）、萨拉拉新日期、PortWatch 9/20 通行 1 艘/日、布油 9/22 97→9/24 106.60→9/25 105.11、SC 9/23 −4%/9/24 +5.02%（媒体收盘）
- 回改标准=官方停火/重开或伊朗许可-收费机制正式落地＋back 回落（[GATE-calm](#r-gate-calm)）→缓和信号·量化未确认/僵局期
- 再袭船·管道再袭＋back 再走阔→维持
- **方向许可不变**：空头[0.3#26](#r-gate-26)否决、多头[0.3#16](#r-gate-16)/[0.3#30](#r-gate-30)（本周 unknown）/[CARD-MA-week](#r-card-ma-week)（本周 unknown）/D8/D12、SC 正套=信号席不建仓、fade（research_only）不适用；9/28 为周末 headline 首个交易日→双向跳空预案
- 供给冲击第四形态"谈判分岔＋制裁升级"（前三形态：油轮战、绕行通道 9/10–11 遇袭/9/17–18 半恢复、护航常态化）仅记录，不新设轴/门
- fed_state=加息落地·连续加息指引（定价加深：9/23 PMI 58.4＋Barr"还需加息"→10 月 FOMC 加息概率 73%（9/23）→71%（9/25）、12 月 95%；10Y 5.22–5.23%（2007 年来最高）；DXY 101.4）
- D13=加息落地·压制逻辑重写（常态扣减 −0.3/×0.5/C 门槛 4.8 照存档；复归档条件=10/27–28 FOMC 按兵或转鸽＋美元连涨中断周度确认，当前更远）
- AU 信号卡=**回吐段·连续加息定价加深**（现货金 9/24 触 4,244 跌破决议后低点约 4,270、9/25 4,280–4,310、周 −1%~−2%以上；黄金 T+D 926.5（截至 9/24 当周 −2.12%）；4404 未回收；<4000 见[PLAN-AU-break](#r-plan-au-break)；"预期兑现型反弹评估"已证伪）
- ②''=形式过门·结构核对未过，第二读数"未修复延续"（沪指本周 9/21–9/24 −0.60%、9/24 3888.37（−1.22%，失守 3900）、深成 −2.37%、创业板 −2.48%；成交 9/23 1.76 万亿、9/24 1.669 万亿未守关口；两融 9/23 沪深京 26,550 亿（−13 亿）；9/15 硬数据弱现实：工业 +5.2%/社零 +0.4%/固投 −7.2%/地产 −19.9%）→×0.5 维持，下一裁决 9/30 9 月 PMI＋10 月中旬硬数据
- ③=判决期（多晶硅减产方案"待最终确认与实施"；钢厂提降准备/第六轮落空为行为性；调减数据见[DYNAMIC-deadline](#r-dynamic-deadline)）
- ④休眠
- RB=旺季证伪侧、多头评估冻结，负反馈检验点=**启动（发起级）**（焦炭第六轮提涨未发起、钢厂第一轮提降 100–110 准备发起，"中秋后实施则国庆前落地"；钢厂盈利率 <10%；按 1.4 行"提涨受阻→启动"；Mysteel 同口径 9/24 当周总库存 1,481.97 万吨（−55.76 去库加速；节前补库）、周产量 779.68（−14.46）；中钢协旬度 −2.8%；下一裁决=提降是否 9/28–9/30 落地：落地→启动确认（原料存量止盈，JM/J 池外无仓）、未落地→维持发起级；"延续"分支须提涨落地）
- 交易所风控=**收紧（全池）**（上期能源/上期所 9/23 起 SC 16–18%/18–20%、RB 7%/9%、AU 16%/18%；大商所 9/29 起 M 8%/10%；郑商所 9/21 起 MA/PTA/PX 2610–2611 分级、9/29 起 MA 10%/9%、CF 9%/8%、SR 8%/7%；见[DYNAMIC-events](#r-dynamic-events)）
- 低敞口=命中（高密度簇＋交易所公告±1日（9/28–9/30 全池）＋D12 高波层（上一实测））
- 本周无行情快照：所有依赖快照的门/读数 unknown（[DYNAMIC-freezes](#r-dynamic-freezes)）
- 例外：以上不是本次已核触发
- 长期许可不得因缺数自行解除
- 到期事件无新证据不得续冻
- 池外品种状态只作背景
- 解除/更新：10/8 长假复盘、10/27–28 FOMC T+1 逐项复评。

<a id="r-dynamic-cpi"></a>
**DYNAMIC-CPI**　利率链分支（"加息＋连续加息指引"分支已于9/17到期执行；当前=连续加息定价加深，10 月 FOMC 三剧本）：

- 已核：加息 25bp 至 3.75–4.00%（12-0；声明"通胀仍然偏高"）、点阵 16/18 年内再加、2026 末中位 4.1%（前 3.8%）；9/23 S&P 闪值 PMI 综合 58.4（62 个月高；服务业 58.7/制造业 56.7；价格指数 2022-10 以来最高）、Fed 理事 Barr"进一步加息很可能是必要的"；10 月再加息 49%（记者会后）→57%（9/18）→73%（9/23）→71%（9/25，目标会议 10/27–28）、12 月 95%；10Y 9/24 峰 5.22%、9/25 5.23%（2007 年来最高）；DXY 9/24 高 101.40；现货金 9/24 触 4,244（跌破决议后低点约 4,270）→9/25 4,280–4,310（周 −1%~−2%以上）；黄金 T+D 926.5（−2.12%）、银 64.04（周约 −4%）
- AU 信号卡=回吐段·连续加息定价加深（"预期兑现型反弹评估"已证伪；4404 未回收）
- 10 月剧本：再加息→压制延续、美元/10Y 新高评估、<4000 破位纪律待命（[PLAN-AU-break](#r-plan-au-break)）
- 按兵或转鸽→D13 复归档议题重启评估前置（需美元连涨中断周度确认）
- 逐项核实际决议/指引、指定会议 FedWatch 采样时刻与价格，再评估 D13
- 例外：金价观察位不授交易许可，不推断现持仓
- AU 不交易、金融属性品种池外→本项无执行表达；9/23 起 AU 涨跌停 16%/保证金 18%
- 不把情景推演写成必然概率
- #29 9/11–9/18 已归档，10/27–28 按 T−10 核重入（国内 T 预计 10/29，待官方日历）
- 解除/更新：10/27–28 FOMC 落地，T+1 重写 fed_state/D13/AU 卡。

<a id="r-dynamic-iron-sample"></a>
**DYNAMIC-iron-sample**　黑色需求侧判决输入；钢材总库存周报（公开来源，须完整年份/观测日/口径；跨年读数已撤回不用）；本周 **Mysteel 五大品种同口径（9/24 当周）已取得**：周产量 779.68 万吨（−14.46）、厂库 382.81（−8.65）、社库 1,099.16（−47.11）、总库存 1,481.97（−55.76，去库加速；与 9/3 同口径 1,572.90 可比）；中钢协 9 月中旬 21 城社库 932 万吨（−2.8%）为第二独立来源；兰格&找钢 9/16 口径（2,101.77 −29.13）不同源不可比；焦炭第六轮未发起、钢厂第一轮提降准备=负反馈启动（发起级）背景；247 家铁水日均/盈利率为参考旁证，不记缺口、不作补核必办；例外：仅依赖该因子的模型必需；不同样本/跨年数据不冒充；去库加速为节前补库，不构成产能性证据或旺季证真；独立国内月差不因缺库存失败，已知反证须处理；RB2701-RB2703 本周分位无快照 unknown，本项仅为背景；解除/更新：下周 Mysteel 公开周报与快照到位后重核。

## 3.4 本月事件与固定风险窗

<a id="r-dynamic-events"></a>
**DYNAMIC-events**　当期事件簇（9/28–10/29；**交易所已核休市日历：9/24、9/30 晚无夜盘；9/25–9/27 中秋休市；10/1–10/7 国庆休市；10/8（周四）集合竞价恢复交易并恢复夜盘**——原"10/1–10/8、10/9 复盘"为日历错误已撤回）：

- **9/28（周一）周末 headline 首个交易日**：伊朗七日方案被拒/经济 D 日/9/21 再袭船/9/24 胡塞袭延布均在闭市期或盘后发生→双向跳空预案（[REG-geop](#r-reg-geop)），不抢跑
- **9/29（周二）郑商所/大商所第二阶段提保扩板**：郑商所 MA 等保证金 10%/涨跌停 9%、CF 9%/8%、SR 8%/7%；大商所 M 等 8%/10%（±1日=9/28–9/30 即假期前全部三个交易日；全池新开门槛+0.3、[REG-exchange](#r-reg-exchange)/[0.3#10](#r-gate-10) ×0.8、公告日核占用）
- 已生效：郑商所 9/21 起 MA/PTA/PX 2610 涨跌停 9%、2611 保证金 9%/涨跌停 8%（"单独通知"即此第一阶段，结案）；上期能源/上期所 9/23 起 SC 涨跌停 16%/套保保证金 17%/一般 18%（SC2610/2611 涨跌停 18%、套保 19%/一般 20%）、RB 涨跌停 7%/套保 8%/一般 9%、AU 涨跌停 16%/套保 17%/一般 18%（"INE 阴性"更新）
- 10/8 恢复交易后自持仓量最大合约未出现涨跌停单边市的首个交易日结算起回落
- **9/30 俄柴油禁令到期**=俄乌轴质变节点（延至 10/31 为报道级、另有"11 月"报道 conflicting、government.ru 最新为 8/29 延至 9/30；官宣日才按[REG-geop](#r-reg-geop)质变日执行）；同日 9 月 PMI 发布
- **9/30 同为国庆长假 T−1**：全品种方向性单边对冲或减至×0.5（[MANAGE-event](#r-manage-event)），[0.3#22](#r-gate-22)跨假期敞口>30% 风险预算减仓
- **10/1–10/7 休市**（10/4 OPEC+ 与地缘 headline 在闭市期发生）；**10/8 首个响应日=复盘日**，按[TRIGGER-D](#r-trigger-d)落地次日确认与双向跳空预案；低敞口条款 10/8 复评；交易所风控回落核
- **10/4 OPEC+**：按兵→延续/增产或闲置产能释放→回吐触发、MA 存量 10/8 检视
- **10/9 USDA WASDE**（美东 12:00=北京 10/10 00:00 周六；国内首次响应 10/12 周一）：M 卡"报告前 2 日禁新开"按国内交易日 10/8–10/9 核、10/12±1 gap 1.3；M 仅观察
- **10/27–28 FOMC**：10 月再加息 71–73%、12 月 95%，三剧本见[DYNAMIC-CPI](#r-dynamic-cpi)
- 官方发布时间与国内 T−n 待官方日历按[CAL-timestamp](#r-cal-timestamp)核（美东 10/28 14:00≈北京 10/29 02:00→国内 T 大概率 10/29）
- AU/AG [0.3#29](#r-gate-29) 按 T−10 核重入
- 已落地归档：9/21 郑商所第一节点、9/23 INE/SHFE/DCE 第一阶段、9/21 海峡两船遇袭（同向质变日±1 已过）、9/25–9/27 中秋休市、9/14 萨拉拉会（推迟无新日期）、9/15 统计局 8 月硬数据、9/17 FOMC、9/16 MA/SC 换月、9/10 PPI、9/11 CPI、9/6 OPEC+、9/10 RB 换月、质变日 9/8–9/9
- 例外：交易所节点为公告日程非发布型事件，不自动打 D12/T−3
- 美国日期/北京时间/国内交易日分列，不以会议首日作 T
- 治理截止/占位窗不是已核市场数据发布
- 解除/更新：各节点落地后复评，10 月官方日历核实后改写日期。

| 事件 | 官方时间 | 北京时间 | 国内交易日T / 处置 |
|---|---|---|---|
| 交易所提保扩板第一阶段（已生效） | 郑商所 2026-09-17 / 上期所·上期能源 2026-09-21 / 大商所 2026-09-18 公告 | 9/21 结算起（MA/PTA/PX 2610–2611）；9/23 结算起（SC、RB、AU；EG） | 已生效；×0.8、公告日核占用 |
| 郑商所/大商所提保扩板第二阶段 | 同上 | 2026-09-29 结算起 | 9/29；MA 10%/9%、CF 9%/8%、SR 8%/7%、M 8%/10%；±1日=9/28–9/30（全池门槛+0.3）；10/8 后按条件回落 |
| 俄柴油禁令到期 / 9 月 PMI / 国庆长假 T−1 | 2026-09-30（政策到期；延期报道级） | 2026-09-30 | 9/30；官宣日=质变日；长假 T−1 对冲或×0.5、#22 跨假期敞口 |
| 中秋 / 国庆休市 | 交易所 2026-09-21 通知 | 9/25–9/27；10/1–10/7 闭市（9/24、9/30 晚无夜盘） | 10/8 集合竞价恢复交易=首个响应日/复盘日/低敞口复评日；假期内 10/4 OPEC+ 与地缘 headline 集中定价 |
| OPEC+ 会议 | 2026-10-04（当地，时刻未核） | 2026-10-04 | 闭市期→10/8 按落地次日确认；MA 存量 10/8 检视 |
| USDA WASDE | 2026-10-09 12:00 美东 | 2026-10-10 00:00（周六） | 10/12；M 报告前 2 日禁新开按国内交易日 10/8–10/9 核；10/12±1 gap 1.3 |
| FOMC 决议 | 2026-10-28 14:00 美东（待官方日历核） | 约 2026-10-29 02:00（待核） | 约 10/29（待核）；T−3/T−1/T+1 下月写死；#29 T−10 |

<a id="r-dynamic-gap"></a>
**DYNAMIC-gap**　金银事件窗口；9/10–9/17=1.3 已到期归档；10/27–28 FOMC±1→1.3（日期待核）；地缘质变±2→1.3；9/23–10/8 提保扩板期按黄金涨跌停 16% 口径核压力场景；化工链 MA：①中断证真·谈判分岔期 1.2 维持（反向质变=官方停火/重开/许可-收费机制落地为主要跳空源，绕行通道再袭（9/24 胡塞袭延布）/修复为第二源），9/21–10/8 提保扩板期与跨 10/1–10/7 长假按扩板后涨跌停口径（甲醇 9%/8%；SC 16–18%）核压力场景；黑色 RB 9/23–10/8 按涨跌停 7% 口径、跨长假从严；农产品 WASDE±1→1.3（10/12±1），9/29–10/8 按 M 8%/CF 8%/SR 7% 口径；例外：AU 信号/AG 休眠仅历史存量；解除/更新：窗口后核。

<a id="r-dynamic-freezes"></a>
**DYNAMIC-freezes**　已过/当前冻结与护栏命中（截至 2026-09-26；**本周 9/21–9/24 无行情快照，最近可用行情日 9/18 已过期**）：

- fixed_risk_window：D12 全品种方向性 9/11–9/17 与 #29 AU/AG 9/11–9/18 均到期归档，下一提前窗待 10 月官方日历
- **#30：MA2701 判定腿 9/21–9/24 逐日结算涨跌未取得（无快照）→本周命中判定 unknown**（主力媒体 9/23 −3%/9/24 +3.26% 与 SC 信号腿不替代判定腿；脚本 v1.16 §2b.1 列已具备，待本机运行）
- SC 信号腿媒体收盘 9/22 夜 −2.3%、9/23 −4%、**9/24 收 730 元/桶 +5.02%**（单日≥5% 记录一次，多向，非结算，信号席仅记录）
- **MA周涨护栏：SC2611 结算周涨 9/17→9/24 无快照→unknown**（上一实测 9/11→9/18 −0.30% 已过期；不以收盘代结算、不判命中或解除）
- **D8（脚本§2d）：本周 unknown**（上一实测 MA 主力上尾 66.7 无档、RB 45.1、SR 下尾 94.1/CF 98.0 空头否决档、AU/SC 无档——过期对照）
- #20：本周 unknown（上一 dist_H250 5.61% 未命中）
- **D12：本周分层与极差 unknown**（上一实测 MA2701 82.9 高波/M2701 99.4/RB 72.7、SR 70.1 常规/CF 22.0、AU 33.0 低波，极差 77.4 错位——过期对照；未经当日重校准的方向性单边隔夜=0）
- 交易所：全池 ×0.8 缓冲（SC/RB/AU 自 9/23、MA 2610/2611 自 9/21、MA/M/SR/CF 自 9/29），公告±1日（9/28–9/30）门槛+0.3
- #16 低敞口限隔夜池方向性单边新开否决当期有效（高密度簇＋交易所公告±1日＋D12 高波层（上一实测））
- #26 能源链方向性空头否决当期有效（①中断证真维持、未证缓和；七日方案被拒非官方重开）
- 质变日：9/21 海峡两船遇袭（UKMTO 多源）=同向质变日，±1 冻结 9/21–9/22 已过（账户未知→条件式）；9/24 胡塞袭延布=绕行通道再袭记录；9/25–26 七日方案/拒绝＋经济 D 日=谈判/制裁 headline，不追加冻结，9/28 双向跳空预案；9/14 萨拉拉推迟与 IRGC 9/17 单源声明未按质变日执行
- 例外：结构 D12/#16/#30 豁免但受组合 cap 与双腿保证金占用
- 提前窗不等实际跨发布
- 已到期冻结不无证续冻
- 无快照不判护栏命中/解除，不沿用上周读数
- 解除/更新：各自期限
- 下次快照重判。

<a id="r-dynamic-deadline"></a>
**DYNAMIC-deadline**　调减数据硬截止已到期；③自然日硬截止2026-09-12当日无可核数据→判决点"无法核验"；首个交易日2026-09-14记录生效；光伏端F腿（SI/PS的F远月表达）新开冻结（中性：非罚则、无叙事扣减、存量不动）；9/23 多晶硅减产方案"待最终确认与实施"、成交好转（N 型复投料 4.30 万、颗粒硅 4.00–4.10 万元/吨）但报价体系未恢复；例外：SI/PS池外，只改本状态与扫描行，不影响池内候选；无数据非未达标；解除/更新：可核数据出现即按[0.3#23](#r-gate-23)原条款裁决。

<a id="r-dynamic-pv-date"></a>
**DYNAMIC-pv-date**　休眠光伏政策纳入口径证据链监测；来源所载2027/1/1实施日期待核；保留未来日期并核正式文件；例外：不替代产能退出/调减达标;不授复活或新开许可；解除/更新：正式文件与实际执行证据到位后按③重核。

## 3.5 未完成事项

<a id="r-dynamic-pending"></a>
**DYNAMIC-pending**　本周缺项（负责人/期限见执行诊断 research/2026-09-26-execution-audit.md）：

- ①**行情快照补跑（首要；账户持有人本机 TUSHARE_TOKEN，2026-09-28 前）**：运行 scripts/future_data.py（v1.17；含 §2b.1 逐日结算与 §5 影子结算）并提交 research/<日期>-data-snapshot.txt；回填 SC2611 结算周涨 9/17→9/24、MA2701 #30、SC 近端 back 方向（①要件二）、A 池三对分位/价差/样本、ATR 分层/极差、D8；无快照则 9/28–9/30 方向性新开判定全部 unknown
- ②MA 国内路线论证已结案（未成案）：复活条件=装置复产数值化/到港回升/太仓现货基差走弱三者之一被观测，且确认定义 A-MA-2701-2705-v1（S 连续 2 个交易日 settle 低于前 10 交易日 S 最高值−0.65×MA2701 ATR20，且 MA2701 未创 H20；effective_from 2026-09-28，前瞻不回判）触发→重开论证；此前保持结案记录，不改名 geopolitical_fade
- ③甲醇港口/华东社会库存同口径数值：连续第三周两入口阴性→降为增强级第二指标（必要独立证据由装置公告/现货基差承担），每周仍尝试两入口、取得即记录，不作必要缺项、不转 research_only
- ④日历纠错写入 canonical/脚本（阶段③/data-sync，已完成）：10/1–10/7、10/8 恢复、9/25–9/27、10/9 WASDE 国内 10/12、10/27–28 国内 T 预计 10/29
- ⑤9/28–9/30 假期前纪律（账户持有人执行/研究方核验，2026-09-30）：9/28 双向跳空预案记录；9/29 第二阶段生效核占用；9/30 T−1 对冲或×0.5、#22、俄禁令官宣核、9 月 PMI
- ⑥①"中断证真"反向监测（下次 change-analysis，2026-10-03）：七日方案后续（官方化/分阶段谈判重启）、胡塞延布损失核实、沙特管道流量、俄禁令 9/30 官宣；back 方向待快照
- ⑦黑色负反馈启动裁决（研究方，2026-10-03）：钢厂第一轮提降是否 9/28–9/30 落地；落地→启动确认（JM/J 池外无仓；RB 多头冻结不变）；未落地→维持发起级
- ⑧CF-autumn B 触发组装（研究方/data_pipeline，next_report）：快照到位后按 SEASONAL_PLAN_INPUT.md 组装 pre_window_low5（8/25–8/31）/[PLAN-B-history](#r-plan-b-history)/MA20/ATR20 并运行 seasonal_plan.py；登记 rule_version=B-v2.24；裁定轮储公告±3 天条款对滚动轮出的适用口径
- ⑨影子计划：本周未新登记（缺 9/24 结算价）；上周 SP-2026-09-19-MA-A-short-spread（+312/+380/+120）与 SP-2026-09-19-MA2701-long-D（3155/3018/3429）有效期 9/25 为休市日（实际最后可入场日 9/24），由脚本§5 按"过入场有效期未成交即作废"机械处理，结算 pending，只供复评 #5/#16/D12
- ⑩当期账户/持仓/挂单/费用/保证金快照（账户持有人，执行核验阶段）
- ⑪MA2705-MA2709 准备对=temporary_gap（上一 MA2709 量缺失、样本 24/21/22；本周无刷新），到期不转 research_only
- SR-summer 已按日历结案（planned_exit 9/22 已过；产销率反向记录维持；9 月产销 10 月初）、CF 供给端反证（储备棉第九周 93.1%、9/20 99.51%、吐絮率近 70%、新棉零星开秤；#5(b) 已核 fail）、M 无许可路由=备选未激活，不作必办
- 例外：池外旧裁决不占当前必办
- 必要未知 incomplete，optional 缺项不冻结
- 到期缺件→下周继续 temporary_gap
- 解除/更新：补件。

## 3.6 当期定性护栏（随周度换版；不改规则本体）

<a id="r-dynamic-overreact"></a>
**DYNAMIC-overreact**　本周不过度反应清单（0.4）与首要纪律风险（6.9）：

- ①伊朗七日重开方案（9/25）≠①缓和证真（美方 9/26 拒绝＋"经济 D 日"、"不在任何谈判中"；反向要件只认官方停火/重开或伊朗许可-收费机制正式落地＋back 回落；9/21 两船遇袭多源=要件一再命中；①维持中断证真、#26 继续、fade 不开放、SC 信号席不建仓）
- ②"经济 D 日"/胡塞袭延布/SC 9/24 +5.02%≠追多许可或 #30 命中判定（+5.02% 为信号腿收盘口径；#30 判定腿 MA2701 逐日结算无快照 unknown；#16 继续；SC 9/23 起提保 18–20%、MA 9/29 起 10%/9%；假期前仅三个交易日、跨 10/1–10/7 闭市）
- ③无本周快照≠上周读数可沿用（分位/D8/周涨/ATR/#30 全部 unknown；不把 RB 56.1/SR 0.0 当本周 no_signal、不把 SC 周涨 −0.30% 当本周未命中；不以媒体收盘代结算；首要动作=补跑 scripts/future_data.py）
- ④金跌破决议后低点 4,270（9/24 4,244）≠C 池空头/趋势级回吐已定/抄底（10 月 71–73% 非 100%；油价-收益率联动可反弹；AU 信号席不交易、#17 不放宽；D13 参数不变、复归档不重启）
- ⑤钢厂第一轮提降准备/第六轮落空≠RB 空头或"原料主线延续"（提降未落地，9/28–9/30 核；RB 空头不在白名单、多头评估冻结不变；Mysteel −55.76 去库加速为节前补库，"延续"分支须提涨落地；JM/J 池外无仓）
- ⑥全池提保扩板（SC/RB/AU 9/23、M/MA/SR/CF 9/29）≠结构豁免或新护栏（既有 ×0.8/门槛+0.3 进入适用；结构不豁免组合上限与双腿占用；1.2 压力场景按新涨跌停板从严；10/8 后条件回落再核）
- ⑦甲醇港口库存数值三周缺≠MA A 不可研究或 research_only（降为增强级第二指标，必要证据由装置公告/现货基差承担：太仓 4,090/西北 3,400/复产慢；国内路线结案是研究结论，复活条件已列）
- ⑧MA 确认定义 v1 冻结≠触发或计划（前瞻 9/28 生效、观测值待快照；国内路线未成案→本轮无计划；不改名 geopolitical_fade；影子计划结算 pending、本周未新登记）
- ⑨沙特管道 9/22 复运＋护航≠绕行通道可靠/供给冲击终结（低速运行、全量需数周、9/24 延布再遭导弹（拦截）；反向"延布出口中断"亦未证实；双向跳空源并存）
- ⑩A 股缩量/沪指失守 3900≠②''修复或出清（成交 1.67–1.76 万亿未守关口；×0.5 维持池外；下一裁决 9/30 PMI＋10 月中旬硬数据）
- ⑪俄禁令"延至 10/31"≠升级已落地（报道级、另有"11 月"报道 conflicting；9/30 节点维持、官宣日=质变日）；light≠稳定性判断（日历纠错＋状态行换版＋适用范围记录＋研究结案，规则本体零改动；快照补跑与 10/8 复盘后级别不预设；护栏命中数/连续空仓/一次踏空不证明规则有效或过严）
- 首要纪律风险（6.9）：在 9/28–9/30 三个交易日、全池提保扩板、无行情快照的窗口用周末 headline（七日方案被拒＋经济 D 日）或 9/24 SC +5%/甲醇 +3.26% 媒体收盘涨跌作方向新开依据并裸持跨 10/1–10/7（9/28 追多：#16、SC 提保、#30 unknown、9/29 MA 提保；追空：#26、方案被拒）、无快照下沿用 9/18 读数判护栏"未命中/解除"、把金跌破 4,270 当趋势级空头或抄底、用提降准备做空黑色、忽视 9/29 公告±1 日门槛+0.3 与 ×0.8（正确=9/30 T−1 对冲或×0.5、#22、10/8 复盘按落地次日确认）、把港口库存缺数当 A 不可研究或把确认定义冻结当触发、未过当日 ATR 重校准隔夜、被动等追保、把池外品种当机会；病根="外部升级、内部收窗"；正确形态="9/28 前补跑快照→存量按护栏与止盈机械执行→9/30 T−1 对冲或×0.5→10/8 复盘"
- 例外：本条不禁止纠错/复评（[AUDIT-rule-review](#r-audit-rule-review)）
- 解除/更新：下周换版。

<a id="r-dynamic-narratives"></a>
**DYNAMIC-narratives**　当期脆弱叙事清单（[PEN-narrative](#r-pen-narrative)评分对象，每条0–3负向）：

- 主="伊朗七日方案＝海峡重开在即、油价见顶"（MA/SC 信号/能源链空头侧；美方 9/26 拒绝＋经济 D 日、9/21 再袭船、PortWatch 1 艘/日、胡塞袭延布；破裂=9/28 开盘双向跳空：谈判死亡＋制裁升级→+5% 级/美方转向接受分阶段方案→反向）
- 新="金跌破决议后低点 4,270＝趋势级回吐已定、抄底/追空皆可"（AU 信号，联动池外金融属性；10 月 71–73% 非 100%、油价-收益率联动可反弹；只影响 fed_state/D13 状态）
- 改版="沙特管道复运＋护航常态化＝绕行通道可靠"（MA/SC 信号；低速运行、全量需数周、9/24 延布再遭导弹；反向"延布出口中断"亦未证实）
- 改版="甲醇现货 4,090/西北 3,400＝近月无限强"与"分位 100＝必回归"双向（MA；复产慢/港口去库无数值 vs 9/29 提保 10%/9%/长假前减仓/9/23 −3%；国内路线结案、v1 前瞻、无本周分位实测）
- 改版="钢厂第一轮提降准备＝黑色见顶做空"（RB；提降未落地、Mysteel −55.76 去库加速/中钢协 −2.8% 节前补库；RB 空头不在白名单）
- 延续="A 股缩量回撤＝资金出清/机会"（内需商品链/权益 beta 池外；成交 1.67–1.76 万亿、创业板周 −2.48%）
- 延续="自律锁价＝价格底/反内卷全面量化"（SI/PS/RB/AL/LC，除 RB 外池外；减产方案待确认）
- 已证伪存档不评分：上期首要"护航 40 艘＋管道半恢复＝海峡事实重开"（以 9/24 胡塞袭延布收场）、"加息落地＝预期兑现型反弹"（以 4,244 收场）、"LC 需求拉动型去库＝重启候选"
- 例外：只对实际适用候选评分
- 池外不入执行分母
- 解除/更新：下周换版。