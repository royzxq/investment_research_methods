...
5. 调用 `future-change-analysis` skill（输入 `AS_OF_DATE`、`CURRENT_META_RESULT`、`PREVIOUS_META_RESULT`、`PREVIOUS_CHANGE_DECISION`、`framework/futures_framework.md` 现行全文），得到 `update_needed`——该 skill 内部有三条触发轴（上期预备观察项复核、周环比变化、框架-现实一致性）外加反摇摆护栏
...
- 只有 `framework/futures_framework.md`（和联动的 `scripts/future_data.py`、衍生的 `framework/futures_framework_compact.md`）的改动需要走分支 + PR；阶段①②的研究日志直接 push 到 main（步骤6），不因为"这周没有框架变化"就不提交
